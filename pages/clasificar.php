<?php
/**
 * Clasificador de Keywords — PHP 7.x compatible
 */
$keywords = []; $results = [];
...
function _clasificar($kw) {
    ...
    $nav = ['/^(mi |mis |acceder a |ingresar a |portal |sucursal |oficina |fono |teléfono |telefono |llamar a |iniciar)/','/\b(oficinas?|sucursales?|teléfonos?|telefonos?|dirección|direccion|ubicación|ubicacion)\b/'];
    foreach ($nav as $p) if (preg_match($p,$kw)) return 'navegacion';
    $trans = ['/\b(comprar|contratar|cotizar|cotiza|precio|cuanto cuesta|cuánto cuesta|valor|costo|presupuesto|tarifa)\b/','/\b(planes?|plan de|seguro|seguros|isapre)\b.*\b(precios?|cotizar|costo|valor|barato|mejor|comparar)\b/','/\b(afilial|afiliarse|cambiarse|cambio de|traslado|trasladarse)\b/','/\b(requisitos para|tramite|trámite|documentos para|como contratar|cómo contratar)\b/','/\b(pre existencias?|preexistencia|enfermedades pre|declaración de salud|declaracion de salud)\b/','/\b(mejor .{1,30} (isapre|seguro|plan)|cual es mejor|cual elegir|comparativa|comparador)\b/','/\b(que isapre|qué isapre|cual isapre|cuál isapre) (elegir|contratar|conviene|recomiendan)\b/','/\b(precio|costo|valor|tarifa|cuota).{1,20}(isapre|seguro|plan|salud)\b/','/\b(isapre|seguro|plan).{1,20}(precio|costo|valor|tarifa|barato)\b/','/\b(donde contratar|dónde contratar|donde cotizar|dónde cotizar|solicitar|pedir|obtener)\b/','/\b(como cambiarse|cómo cambiarse|cambio de isapre|cambio de plan)\b/','/\b(afiliar a mi|inscribir a mi|agregar a mi|incluir a mi)\b/','/how much .*(insurance|health|plan)/','/\b(buy|purchase|get|find|compare|switch).*(insurance|health|plan|coverage)\b/','/\b(planes para|plan para).{1,30}\b/','/\b(seguro para|isapre para).{1,30}\b/'];
    ...
    $clean = preg_replace('/[\.\-\s]/','',$kw);
    foreach ($brands as $b) { if ($clean === preg_replace('/\s/','',$b)) return 'navegacion'; }
    return 'informativa';
}
