<?php
/**
 * Funciones del child theme del blog PSF.
 */
...
add_filter('render_block', function (string $block_content, array $block): string {
    ...
    if ($class_name && strpos($class_name, 'ri-blog-header-right') !== false && strpos($block_content, 'ri-blog-follow') === false) {
        return preg_replace(
    ...
    if (($block['blockName'] ?? '') === 'core/template-part' && ($block['attrs']['slug'] ?? '') === 'footer' && strpos($block_content, 'ri-blog-follow') === false) {
        return $block_content . psf_blog_follow_markup();
    }
