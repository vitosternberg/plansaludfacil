...
<div class="relative mb-12 md:mb-32">
    ...
    <div class="relative md:absolute left-0 right-0 -mt-24 md:mt-0 md:-bottom-12 z-20 px-4">
        <form id="hero-cotizador" class="max-w-5xl mx-auto bg-white rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.15)] p-4 md:p-6 border border-gray-100 flex flex-col md:flex-row items-center justify-between gap-4 transition-all relative">
            ...
            <p id="hero-error" class="hidden text-xs text-red-300 font-medium mb-1"></p>
        ...
        function heroValidar(){const r=document.getElementById('hero-renta').value.trim();const e=document.getElementById('hero-edad').value.trim();const c=parseInt(document.getElementById('hero-cargas').value)||0;const err=document.getElementById('hero-error');if(!r){err.textContent='Ingresa tu renta líquida';err.classList.remove('hidden');return false}if(!e){err.textContent='Ingresa tu edad';err.classList.remove('hidden');return false}if(c>0){let falta=false;document.querySelectorAll('.hero-carga-edad').forEach(el=>{if(!el.value.trim())falta=true});if(falta){err.textContent='Ingresa la edad de todas las cargas';err.classList.remove('hidden');return false}}err.classList.add('hidden');return true}
        function heroCotizar(){if(!heroValidar())return;const r=document.getElementById('hero-renta').value||'0';const e=document.getElementById('hero-edad').value||'';const c=document.getElementById('hero-cargas').value||'0';const p=new URLSearchParams({age:e,income:r,cargas:c});if(c>0){document.querySelectorAll('.hero-carga-edad').forEach((el,i)=>{if(el.value)p.append('carga_edad[]',el.value)})};const d=c>0?BASE_URL+'/planes/familiares/':BASE_URL+'/planes/individuales/';window.location.href=d+'?'+p.toString()+'#formulario'}
        function heroCotizarExpress(){if(!heroValidar())return;const r=document.getElementById('hero-renta').value.trim();const e=document.getElementById('hero-edad').value.trim();const c=document.getElementById('hero-cargas').value||'0';const p=new URLSearchParams({edad:e,renta:r,cargas:c});if(c>0){document.querySelectorAll('.hero-carga-edad').forEach((el,i)=>{if(el.value)p.append('carga_edad[]',el.value)})};window.location.href=BASE_URL+'/planes/comparador/?'+p.toString()+'#comparador'}
        </script>
    </div>
</div>
